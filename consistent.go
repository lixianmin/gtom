...
func ConsistentHashFilterFromFile(name string, configFile string) (OpFilter, error) {
...
func ConsistentHashFilterFromDocument(name string, document map[string]interface{}) (OpFilter, error) {
...
func ConsistentHashFilter(name string, workers []string) (OpFilter, error) {
	if len(workers) == 0 {
		return nil, EmptyWorkers
	}
	found := false
